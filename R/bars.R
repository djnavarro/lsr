...
bars <- function(
  ...
  if (!all(sapply(gpName, function(x) {
    methods::is(data[, x], "factor")
  }))) {
  ...
  nLevels <- sapply(gpName, function(x) {
    nlevels(data[, x])
  })
  ...
  # barplot
  xloc <- graphics::barplot(
    height = h, beside = b, legend.text = FALSE, width = 1,
  ...
  if (showErrorBars) {
    ...
    for (k in 1:dim(errorBars)[1]) {
      addOneErrorBar(
        xloc[k], barHeights[[nFactors + 1]][i],
  ...
  out <- cbind(
    barHeights[, 1:nFactors, drop = FALSE],
