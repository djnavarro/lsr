...
who <- function(expand = FALSE) {
  ...
  getWhoInfo <- function(varnames, envir, is.df) {
    ...
    for (i in 1:n) {
      if (is.df) {
        var <- varnames[i]
        var_call <- call("$", as.name(df), as.name(var)) # construct a call
        v <- eval(var_call, envir) # evaluate call (i.e., extract variable)
      } else {
        v <- eval(as.name(varnames[i]), envir)
      }
